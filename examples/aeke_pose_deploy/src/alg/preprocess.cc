...
void letterbox(const cv::Mat &image, cv::Mat &padded_image, BOX_RECT &pads,
  ...
  // 在图像周围添加填充
  cv::copyMakeBorder(resized_image, padded_image, pads.top, pads.bottom,
                     pads.left, pads.right, cv::BORDER_CONSTANT, pad_color);
}
...
int resize_rga(rga_buffer_t &src, rga_buffer_t &dst, const cv::Mat &image,
  ...
  src = wrapbuffer_virtualaddr((void *)image.data, img_width, img_height,
                               RK_FORMAT_RGB_888);
  dst = wrapbuffer_virtualaddr((void *)resized_image.data, target_width,
...
cv::Mat convertYUVtoJpg(const void *yuvData, int width, int height,
  ...
  const unsigned char *yuvList = static_cast<const unsigned char *>(yuvData);
  std::string input(yuvList, yuvList + size);
  // printf("size:%zu\n",size);
  ...
  std::stringstream ss(input); // 使用stringstream进行分割
  int value;
  char comma; // 用于读取逗号
  while (ss >> value) {
  ...
  std::vector<int> yList(intVector.begin(), intVector.begin() + ySize);
  std::vector<int> uvList(intVector.begin() + ySize,
                          intVector.begin() + ySize + uvSize);
  std::vector<int> yuv(yList);
  yuv.insert(yuv.end(), uvList.begin(), uvList.end());

  // 转换为 unsigned char 类型
  std::vector<unsigned char> yuvUchar(yuv.begin(), yuv.end());

  cv::Mat yuvMat(height * 3 / 2, width, CV_8UC1, yuvUchar.data());
  cv::Mat rgbMat;
...
static int crop_and_scale_image_c(int channel, unsigned char *src,
                                  int src_width, int src_height, int crop_x,
                                  int crop_y, int crop_width, int crop_height,
                                  unsigned char *dst, int dst_width,
                                  int dst_height, int dst_box_x, int dst_box_y,
...
static int crop_and_scale_image_yuv420sp(unsigned char *src, int src_width,
                                         int src_height, int crop_x, int crop_y,
                                         int crop_width, int crop_height,
                                         unsigned char *dst, int dst_width,
                                         int dst_height, int dst_box_x,
                                         int dst_box_y, int dst_box_width,
                                         int dst_box_height) {
...
int convert_image(image_buffer_t *src_img, image_buffer_t *dst_img,
  ...
  printf("src width=%d height=%d fmt=0x%x virAddr=0x%p fd=%d\n", src_img->width,
         src_img->height, src_img->format, src_img->virt_addr, src_img->fd);
  printf("dst width=%d height=%d fmt=0x%x virAddr=0x%p fd=%d\n", dst_img->width,
         dst_img->height, dst_img->format, dst_img->virt_addr, dst_img->fd);
  if (src_box != NULL) {
    printf("src_box=(%d %d %d %d)\n", src_box->left, src_box->top,
           src_box->right, src_box->bottom);
  }
  if (dst_box != NULL) {
    printf("dst_box=(%d %d %d %d)\n", dst_box->left, dst_box->top,
           dst_box->right, dst_box->bottom);
  }
  printf("color=0x%x\n", color);
...
cv::Mat convert(jbyte *yuvData) {
  cv::Mat yuvMat(480 * 3 / 2, 640, CV_8UC1, yuvData);
  cv::Mat rgbMat;
  cv::cvtColor(yuvMat, rgbMat, cv::COLOR_YUV2RGB_NV21);
  return rgbMat;
}
...
cv::Mat rotate(cv::Mat rgbMat) {
  int center_x = rgbMat.cols / 2;
  int center_y = rgbMat.rows / 2;
  cv::Mat rotation_matrix =
      cv::getRotationMatrix2D(cv::Point(center_x, center_y), -90, 1.0);
  cv::Mat rot_mat;
  cv::warpAffine(rgbMat, rot_mat, rotation_matrix,
                 cv::Size(rgbMat.rows, rgbMat.cols));
  return rgbMat;
}
